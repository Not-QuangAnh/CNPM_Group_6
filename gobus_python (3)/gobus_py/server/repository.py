...
def get_user_by_email(email):
    rows = run_query("SELECT id, name, email, phone, password_hash FROM dbo.Users WHERE email=?", (email,))
    return rows[0] if rows else None


def get_user_by_id(user_id):
    rows = run_query("SELECT id, name, email, phone FROM dbo.Users WHERE id=?", (user_id,))
    return rows[0] if rows else None
...
def create_promo(data):
    ...
    run_command(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (
            promo_id, code.upper(), desc, data.get("discount", "..."), 0,
